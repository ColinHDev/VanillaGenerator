...
class NetherGenerator extends VanillaGenerator
{
	...
	protected function generateChunkData(ChunkManager $world, int $chunk_x, int $chunk_z, VanillaBiomeGrid $biomes): void
	{
		...
		$octaves->surface->getFractalBrownianMotion($surface_noise, $cx, $cz, 0, 0.5, 2.0);
		$octaves->soul_sand->getFractalBrownianMotion($soul_sand_noise, $cx, $cz, 0, 0.5, 2.0);
		$octaves->gravel->getFractalBrownianMotion($grave_noise, $cx, 0, $cz, 0.5, 2.0);
	...
	protected function createWorldOctaves(): NetherWorldOctaves
	{
		...
		$surface->setXScale(static::SURFACE_SCALE);
		$surface->setYScale(static::SURFACE_SCALE);
		$surface->setZScale(static::SURFACE_SCALE);
